...
public class ComplexAOIRelationFinder implements Finder<ComplexAOIRelation>
{
    ...
    {
        return Iterables.asIterable(StreamSupport.stream(atlas.relations().spliterator(), false)
                .map(ComplexAOIRelation::getComplexAOIRelation).filter(Optional::isPresent)
