...
public class RawAtlasGenerator
{
    ...
    private Atlas buildRawAtlas()
    {
        ...
        final Atlas atlas;
        if (!this.pbfReader.getPointIdentifiersToFilter().isEmpty())
        {
            // Filter out any points that we don't need as a result of filtering lines
            atlas = this.builder.get()
                    .subAtlas(entity -> !(entity instanceof Point && this.pbfReader
                            .getPointIdentifiersToFilter().contains(entity.getIdentifier())))
                    .get();
        }
        else
        {
