...
public enum Dimensions
{
    ...
    public static Bbox toBbox(final Double... coordinates)
    {
        ...
        final Dimensions dimensions = Arrays.stream(Dimensions.values()).filter(
                theseDimensions -> theseDimensions.getNumberOfCoordinates()
                        == length).findFirst().orElseThrow(
                () -> new IllegalArgumentException(Arrays.toString(coordinates)));
